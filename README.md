...
- Envio automático de cada chamado copiado como arquivo `.txt` para uma pasta do Google Drive.
...
O app usa OAuth do Google (client-side, via [Google Identity Services](https://developers.google.com/identity/oauth2/web/guides/overview)) para enviar cada chamado copiado como um arquivo de texto para a pasta do Drive configurada. Para ativar:
...
A partir daí, todo chamado copiado é enviado automaticamente para a pasta do Drive configurada em `GOOGLE_DRIVE_FOLDER_ID` (já configurada para [esta pasta](https://drive.google.com/drive/folders/1WrUxMv-tASmF0etoj8HLoIO4nKvTr4OD)). Chamados gerados antes de conectar, ou que falharem no envio, mostram um aviso e podem ser reenviados manualmente pelo botão dentro do modal "Detalhes do Log".
